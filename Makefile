...
# 编译器是Verilator
VERILATOR 				:= CC=$(CC) CXX=$(CXX) verilator
GTKWAVE					:= gtkwave
CC 						:= ccache gcc
CXX						:= ccache g++
...
CFLAGS 					+= $(INCFLAGS) -g
#CFLAGS 					+= -O3
CFLAGS 					+= $(filter-out -D__STDC_FORMAT_MACROS, $(shell llvm-config --cxxflags)) -fPIE
#CFLAGS 					+= -fuse-ld=mold
...
# 运行时参数和img文件
#NPC_FLAGS   		+= -b
NPC_FLAGS 			+= -l $(shell dirname $(IMG).bin)/npc-log.txt 
NPC_FLAGS 			+= -e $(addsuffix .elf,$(basename $(IMG))) 
NPC_IMAGE 			+= $(IMG)
...
#include $(TJUTCORE_HOME)/../sta/sta.mk

# compile
compile: $(CSRCS) $(VSRCS)
	$(Q)echo "# Building npc [riscv32-npc]"
...
# wave
wave: 
	$(GTKWAVE) $(BUILD_DIR)/npc_wave.vcd
